...
def test_same_week_day_hour_lag():
    dates = pd.Series([datetime.datetime(2000, 1, 1) + datetime.timedelta(days=x) for x in range(1200)])
    vals = pd.Series(x for x in range(1200))

    df = same_week_day_hour_lag(dates, vals)
    assert len(df.columns) == 1
    assert len(df["SameWeekHourLag"]) == 1200

    dfq = same_week_day_hour_lag(dates, vals, agg_func="quantile", q=0.5)
    assert len(dfq.columns) == 1
    assert len(dfq["SameWeekHourLag"]) == 1200

    dfs = same_week_day_hour_lag(dates, vals, agg_func="std")
    assert len(dfs.columns) == 1
    assert len(dfs["SameWeekHourLag"]) == 1200

    with pytest.raises(Exception):
        same_week_day_hour_lag(dates, vals, agg_func="stdev")

def test_same_day_hour_lag():
    dates = pd.Series([datetime.datetime(2000, 1, 1) + datetime.timedelta(days=x) for x in range(1200)])
    vals = pd.Series(x for x in range(1200))

    df = same_day_hour_lag(dates, vals)
    assert len(df.columns) == 1
    assert len(df["SameDayHourLag"]) == 1200

    dfq = same_day_hour_lag(dates, vals, agg_func="quantile", q=0.5)
    assert len(dfq.columns) == 1
    assert len(dfq["SameDayHourLag"]) == 1200

    dfs = same_day_hour_lag(dates, vals, agg_func="std")
    assert len(dfs.columns) == 1
    assert len(dfs["SameDayHourLag"]) == 1200

    with pytest.raises(Exception):
        same_day_hour_lag(dates, vals, agg_func="stdev")

def test_same_day_hour_moving_average():
    dates = pd.Series([datetime.datetime(2000, 1, 1) + datetime.timedelta(days=x) for x in range(1200)])
    vals = pd.Series(x for x in range(1200))
    df = same_day_hour_moving_average(dates, vals, start_week=1, window_size=5, average_count=1,
                                      forecast_creation_time=datetime.datetime(2001, 1, 1))
    assert len(df.columns) == 1

def test_same_day_hour_moving_quantile():
    dates = pd.Series([datetime.datetime(2000, 1, 1) + datetime.timedelta(days=x) for x in range(1200)])
    vals = pd.Series(x for x in range(1200))
    df = same_day_hour_moving_quantile(dates, vals, start_week=1, window_size=5, quantile_count=1, q=0.5,
                                       forecast_creation_time=datetime.datetime(2001, 1, 1))
    assert len(df.columns) == 1

def test_same_day_hour_moving_std():
    dates = pd.Series([datetime.datetime(2000, 1, 1) + datetime.timedelta(days=x) for x in range(1200)])
    vals = pd.Series(x for x in range(1200))
    df = same_day_hour_moving_std(dates, vals, start_week=1, window_size=5, std_count=1,
                                  forecast_creation_time=datetime.datetime(2001, 1, 1))
    assert len(df.columns) == 1

def test_same_day_hour_moving_agg():
    dates = pd.Series([datetime.datetime(2000, 1, 1) + datetime.timedelta(days=x) for x in range(1200)])
    vals = pd.Series(x for x in range(1200))
    df = same_day_hour_moving_agg(dates, vals, start_week=1, window_size=5, count=1,
                                  forecast_creation_time=datetime.datetime(2001, 1, 1), agg_func="mean")
    assert len(df.columns) == 1
