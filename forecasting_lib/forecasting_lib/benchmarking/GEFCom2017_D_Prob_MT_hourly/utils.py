...
from benchmark_settings import TRAIN_BASE_END, TRAIN_ROUNDS_ENDS, TEST_STARTS_ENDS
...
def split_train_test(
    ...
    for i in range(len(TRAIN_ROUNDS_ENDS)):
        file_name = os.path.join(train_data_dir, train_file_prefix + str(i + 1) + ".csv")
        train_round_delta_df = full_df.loc[(index_value >= TRAIN_BASE_END) & (index_value < TRAIN_ROUNDS_ENDS[i])]
        print("Round {0} additional training data size: {1}".format(i + 1, train_round_delta_df.shape))
        print("Minimum timestamp: {0}".format(min(train_round_delta_df.index.get_level_values(0))))
        print("Maximum timestamp: {0}".format(max(train_round_delta_df.index.get_level_values(0))))
        print("")
        train_round_delta_df.to_csv(file_name)

    for i in range(len(TEST_STARTS_ENDS)):
        test_file = os.path.join(test_data_dir, test_file_prefix + str(i + 1) + ".csv")
        ground_truth_file = os.path.join(ground_truth_dir, test_file_prefix + str(i + 1) + ".csv")
        start_end = TEST_STARTS_ENDS[i]
        test_round_df = full_df.loc[((index_value >= start_end[0]) & (index_value < start_end[1]))].copy()

        test_round_df.to_csv(ground_truth_file)
        test_round_df.drop(DROP_COLUMNS, inplace=True, axis=1)
        test_round_df.to_csv(test_file)

        print("Round {0} testing data size: {1}".format(i + 1, test_round_df.shape))
        print("Minimum timestamp: {0}".format(min(test_round_df.index.get_level_values(0))))
        print("Maximum timestamp: {0}".format(max(test_round_df.index.get_level_values(0))))
        print("")
